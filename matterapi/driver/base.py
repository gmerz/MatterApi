...
logger = logging.getLogger("matterapi.driver")
...
class DriverOptions(BaseModel):
    ...
    """ Authentication information for the driver

    Provide login information for task which require authentication.
...
class BaseDriver(BaseModel, metaclass=abc.ABCMeta):
    ...
    user: Optional[User]
    """ Object to hold information on the current user/bot. Will be set populated after calling login() """
    ...
    async def start_ws(self, event_handler: Callable, relogin=False):
        ...
        while True:
            try:
                ...
                logger.exception("Got an unexpected exception from websockts")
